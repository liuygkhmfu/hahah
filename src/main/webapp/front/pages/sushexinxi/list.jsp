			...
			<div class="recommend" :style='{"margin":"10px auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
				...
				<form class="layui-form filter" :style='{"padding":"0px","margin":"20px 7% 20px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>宿舍名称</div>
						<input type="text" :style='{"padding":"0 10px","margin":"0","outline":"none","borderColor":"#ddd","color":"#666","borderRadius":"0px","borderWidth":"1px","width":"140px","fontSize":"14px","lineHeight":"40px","borderStyle":"solid","height":"40px"}' name="sushemingcheng" id="sushemingcheng" placeholder="宿舍名称" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>宿舍类型</div>
						<select name="susheleixing" id="susheleixing" lay-filter="susheleixing">
							<option value="">请选择</option>
							<option v-for="(item,index) in susheleixingOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>有床位</div>
						<select name="youchuangwei" id="youchuangwei" lay-filter="youchuangwei">
							<option value="">请选择</option>
							<option v-for="(item,index) in youchuangweiOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
				...
				<div :style='{"width":"100%","margin":"20px 0 10px","minWidth":"1050px","background":"none","flex":"1","order":"3"}' class="lists">
					...
					<div :style='{"padding":"0px 7%","margin":"0","flexWrap":"wrap","background":"none","display":"block","width":"100%","justifyContent":"space-between","height":"auto"}' class="list list-1">
						<div :style='{"border":"0px solid #e6e6e6","padding":"0px","margin":"0 0 40px 2%","borderRadius":"0","flexWrap":"wrap","background":"rgba(245,245,245,.0)","display":"flex","width":"31%","position":"relative","float":"left","justifyContent":"space-between","height":"auto"}' @click="jump('../sushexinxi/detail.jsp?id='+item.id)" v-for="(item,index) in dataList" :key="index" class="list-item animation-box">
		...
		<script>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				http.request('config/list', 'get', {
			        ...
			        vue.susheleixingOptions = '男生,女生'.split(',')
					vue.$nextTick(() => {form.render()})
			        vue.youchuangweiOptions = '是,否'.split(',')
					vue.$nextTick(() => {form.render()})
				...
				function pageList() {
			        ...
			        if (jquery('#sushemingcheng').val()) {
			          param['sushemingcheng'] = jquery('#sushemingcheng').val() ? '%' + jquery('#sushemingcheng').val() + '%' : '';
			        }
			        if (jquery('#susheleixing').val()) {
			          param['susheleixing'] = jquery('#susheleixing').val() ? jquery('#susheleixing').val() : '';
			        }
			        if (jquery('#youchuangwei').val()) {
			          param['youchuangwei'] = jquery('#youchuangwei').val() ? jquery('#youchuangwei').val() : '';
			        }
				...
				if (jquery('#youchuangwei').val()) {
					...
					// 获取列表数据
					http.request('sushexinxi/list', 'get', param, function(res) {
